...
class MyDataset(Dataset):
    def __init__(self, data, label, exp_type):
        self.data = data
        self.label = self.alter_label(label, exp_type)
    ...
    def alter_label(self, labels, exp_type):
        # classfication
        #out_labels = np.zeros_like(labels)
        # ordinal
        if exp_type == 0:
            out_labels = np.zeros_like(np.arange(labels.shape[0] * 4).reshape(labels.shape[0], 4))
        else:
        ...
        for i in range(labels.shape[0]):
            ...
            if exp_type == 0:
                if labels[i] == 0:
                    ...
                    # classification
                    out_labels[i] = np.array([1, 0, 0, 0])
                ### low risk
                elif labels[i] ==  1:
                    # ordinal
                    # out_labels[i] = np.array([1, 0, 0])
                    # classification
                    out_labels[i] = np.array([0, 1, 0, 0])
                ### medium risk
                elif labels[i] == 2:
                    # ordinal
                    # out_labels[i] = np.array([1, 1, 0])
                    # classification
                    out_labels[i] = np.array([0, 0, 1, 0])
                ### high risk
                elif labels[i]== 3:
                    # ordinal
                    # out_labels[i] = np.array([1, 1, 1])
                    # regression/classification
                    out_labels[i] = np.array([0, 0, 0, 1])
            else:
                if labels[i] == 0:
                    # ordinal
                    out_labels[i] = np.array([0,0,0])
...
class BasicBlock(nn.Module):
    ...
    params:
        ...
        color_groups: number of color_groups in convk
    ...
    def __init__(self, in_channels, out_channels, ratio, kernel_size, stride, groups, downsample, is_first_block=False, use_bn=True, use_do=True):
        ...
        # self.se_avp = nn.AdaptiveAvgPool1d(1)
        # C = self.out_channels
        # t = int(abs((math.log(C, 2) + 1) / 2))
        # k = t if t % 2 else t + 1
        # self.se_conv = nn.Conv1d(1, 1, kernel_size=k, padding=int(k / 2), bias=False)
        #
        if self.downsample:
    ...
    def forward(self, x):
        ...
        # y = self.se_avp(out)
        # y = self.se_conv(y.transpose(-1,-2))
        # y = y.transpose(-1,-2)
    ...
    def forward(self, x):
        ...
        for i_block in range(self.m_blocks):
            ...
            if self.verbose:
                ...
                print('stage: {}, block: {}, conv1: {}->{} k={} s={} C={}'.format(self.i_stage, i_block, net.conv1.in_channels, net.conv1.out_channels, net.conv1.kernel_size, net.conv1.stride, net.conv1.color_groups))
                print('stage: {}, block: {}, convk: {}->{} k={} s={} C={}'.format(self.i_stage, i_block, net.conv2.in_channels, net.conv2.out_channels, net.conv2.kernel_size, net.conv2.stride, net.conv2.color_groups))
                print('stage: {}, block: {}, conv1: {}->{} k={} s={} C={}'.format(self.i_stage, i_block, net.conv3.in_channels, net.conv3.out_channels, net.conv3.kernel_size, net.conv3.stride, net.conv3.color_groups))
